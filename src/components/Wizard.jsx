...
const Wizard = () => {
    ...
    return (
        <Container>
            <Title>Les trois sorciers principaux</Title>
            <ContainerCart>
                {!wizards ? '' : wizards.filter((heros) => 
                    heros.name === 'Harry Potter' || 
                    heros.name === 'Hermione Granger' || 
                    heros.name === 'Ron Weasley').map((hero) => (
                    <Cart>
                        <WizardName>{hero.name}</WizardName>
                        <Image src={hero.image} alt={hero.name} />
                        <Info> Acteur : {hero.actor}</Info>
                        <Info> Maison: {hero.house}</Info>
